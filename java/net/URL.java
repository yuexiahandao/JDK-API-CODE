 ...
 * 例如对数据库或搜索引擎的查询。有关 URL 的类型和格式的更多信息，可从以下位置找到：
 * http://www.socs.uts.edu.au/MosaicDocs-old/url-primer.html
 *
 * 通常，URL 可分成几个部分。上面的 URL 示例指示使用的协议为 http （超文本传输协议）并且该信息驻留在一台名为 www.socs.uts.edu.au 的主机上。
 * 主机上的信息名称为 /MosaicDocs-old/url-primer.html。主机上此名称的准确含义取决于协议和主机。
 * 该信息一般存储在文件中，但可以随时生成。该 URL 的这一部分称为路径 部分。
 * <p>
...
public final class URL implements java.io.Serializable {
     ...
     *
     * 指定可以被协议处理器扫描的包前缀列表的属性。 这个属性的值应该是以竖线分隔的包名称列表，用来给协议处理器加载。
     * 这个类的策略是所有的协议处理器将被放置在一个叫protocolname的处理器类中，另外列表中的包是将依次检查匹配处理器。
     * 如果没有找到或者这个属性没有指定值，默认的包前缀sun.net.www.protocol会被使用。搜索的过程是从头到尾，
     * 找到匹配的处理器就停止。
     *
     */
     ...
     * 主机的ip地址，在equals和hashCode方法中使用，这个很妙！
     * 按需计算。未初始化的或未知的主机地址为空。
     */
